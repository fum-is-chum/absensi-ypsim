...
class _PengajuanIzin extends State<PengajuanIzin> {
  ...
  final GlobalKey<_LampiranField> _lampiranKey =
      new GlobalKey<_LampiranField>();
                            ...
                            child: ElevatedButton(
                                onPressed: () async {
                                  if (_key.currentState!.validate()) {
                                    ...
                                    try {
                                      if (_isEdit) {
                                        if (_hasFile && _fileLoading)
                                          throw 'File Loading';
                                        if (await _pengajuanIzinBloc
                                            .updateIzin(context)) {
...
class _TanggalField extends State<TanggalField> {
  ...
  void initState() {
    ...
    _controller.text = widget.isAkhir
        ? _pengajuanIzinBloc.model.end_date
        : _pengajuanIzinBloc.model.start_date;
  }
  ...
  void _showDatePicker() {
    showDatePicker(
            ...
            firstDate: widget.isAkhir
                ? DateTime.parse(_pengajuanIzinBloc.model.start_date)
...
class _KeteranganField extends State<KeteranganField> {
  ...
  Widget build(BuildContext context) {
    return TextFormField(
      ...
      decoration: InputDecoration(
          hintText: 'Keterangan Izin', border: OutlineInputBorder()),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        Text(
          'Ekstensi file: .pdf, .docx, .doc, .png, .jpg, .jpeg\nUkuran maks: 2 MB',
          style: TextStyle(fontSize: 12, color: MaterialColors.muted),
        )
      ],
    );
  }
